...
- The `<form>` tag has empty `action` and `method` attributes because this is a purely front-end exercise.
...
- There is no JavaScript build step because there is little benefit for either myself or a reviewer beyond a small performance gain. Therefore, the source files live at the root because HTML and JS require no compilation; only CSS is compiled via Sass.
...
I have used Lighthouse for a quality/performance audit to enforce basic performance, accessibility, and SEO standards. Although I'm aware this is no substitute for RUM via CrUX, etc.
